...
useEffect(() => {
  ...
  // Reload whenever a new order is created
...
const orders=myOrders.filter(order=>order.userEmail === currentUser.email);


const pending=orders.filter((item)=>item.status === "Pending");
const completed=orders.filter((item)=>item.status === "Competed");
